...
def test_rejects_invalid_input_without_touching_gee(failure):
    ...
    if failure == "missing_pre":
        pre = None
    if failure == "missing_post":
        post = None
    if failure == "foreign_event":
        images[11].event_id = 2
    if failure == "wrong_phase":
        images[11].phase = "pre"
    if failure == "foreign_aoi":
        aoi.event_id = 2
    if failure == "local_raster":
        images[11].source_kind = "local_upload"
    if failure == "wrong_sensor":
        images[11].satellite = "Landsat"
    if failure == "wrong_resolution":
        images[11].resolution_m = 20
    if failure == "date_order":
        images[11].acquisition_date = images[10].acquisition_date
    if failure == "disabled":
        model = "building_segmentation_v1"
    with patch.object(service.disaster_repo, "get_event", return_value=event), patch.object(service.disaster_repo, "get_aoi", return_value=aoi), patch.object(service.disaster_repo, "get_imagery", side_effect=lambda db, key: images.get(key)):
        with pytest.raises(AnalysisError):
            service.validate_inputs(None, 1, 1, pre, post, model)
...
def test_invalid_or_empty_raster_never_succeeds(groups):
    with pytest.raises(AnalysisError):
        class_statistics(get_model("dynamic_world_v1")["classes"], groups, groups, 100)
...
def test_processing_run_cannot_start_twice():
    with patch.object(service.disaster_repo, "get_run", return_value=Row(status="processing")):
        with pytest.raises(AnalysisError, match="masih diproses"):
            service.run_analysis(MagicMock(), 1)
